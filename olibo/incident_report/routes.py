...
from olibo.match_sheet.model import Match              # ← from ast import Match corrigé
from olibo.team.model import TeamMember                # ← pour valider le member_id
...
def create_incident_report():
    try:
        ...
        if user.role not in ['referee', 'commissioner']:
            return jsonify({'error': 'Only referees and commissioners can report incidents'}), 403
        ...
        # Validation optionnelle du membre impliqué
        ...
        report = IncidentReport(
            ...
            reporter_id=user.id,
            member_id=member_id,                  # ← player_id → member_id
...
def update_incident_report(report_id):
    try:
        ...
        if 'status' in data:
            if data['status'] not in ('reported', 'under_review', 'resolved'):
