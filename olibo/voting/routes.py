...
from olibo.team.model import TeamMember          # ← Player → TeamMember
...
def cast_vote():
    try:
        ...
        # On ne vote que pour des joueurs
        if not member.is_player:
            return jsonify({'error': 'You can only vote for players'}), 400

        # Vérification vote unique par catégorie/période
        ...
        vote = Vote(
            voter_id=voter_id,
            member_id=data['member_id'],          # ← player_id → member_id
            competition_id=data['competition_id'],
            vote_type=data['vote_type'],
            matchday=data.get('matchday'),
        )
        db.session.add(vote)
        db.session.flush()  # on persiste le vote avant de mettre à jour les résultats

        # Mise à jour du compteur dans VoteResult
...
def delete_vote(vote_id):
    try:
        ...
        # Décrémente le compteur associé
        result = VoteResult.query.filter_by(
